...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modulesProduct = module1 * module2;
  const cos = scalarProduct / modulesProduct;
...
function roundToPowerOfTen(num, pow) {
  ...
  let temp = '1'; /* don't know how to name variable */
...
function toNumber(value, def) {
  ...
  return Number.isInteger(valueNum) ? valueNum : def;
}
...
function getSumOfDigits(num) {
  ...
  const sum = numArray.reduce((acc, digit) => {
    return acc + +digit;
  }, 0);
  return sum;
}
...
function getCountOfOddNumbers(number) {
  ...
  const numModule = Math.abs(number);
  return numModule % 2 === 0 ? numModule / 2 : (numModule + 1) / 2;
}
